...
Provider 与 Analytics 没有平台数据库写权限。Analytics 的成绩源账号只允许对 `grade_details` 执行聚合所需的只读查询；发布前校验快照，发布后只读不可变批次。后续排名、分位数和更多聚合应在 Analytics 内增加模块与 RPC operation，不把明细成绩传回平台。
