...
本科生选课结果补充查询已经将“抽签落选”“个人退选”“管理员退选”解析为 `failed`，但部分前端只直接展示 `schedule` 字段，无法看到失败原因。需要在 provider 解析阶段把失败原因写入 `schedule`，失败记录不再保留原始上课时间。
...
- 仅调整 OUC 本科生选课结果解析。
- 当记录为失败状态且存在结果文本时，用结果文本替换 `schedule`。
- 无论原始 `sksj` 是否为空，失败记录的 `schedule` 都直接使用失败原因。
- 正常已选、待确认记录保持现状。
